...
static void createMeshesFromFBX(const aiScene& scene, std::vector<std::shared_ptr<Model>>& models) {
	...
	for (unsigned int i = 0; i < scene.mNumMeshes; i++) {
		...
		modelsData[i] = std::make_shared<ModelData>();
		models[i] = std::make_shared<Model>();
		//models[i]->SetMeshName(mesh->mName.data);
...
GameObject gameObjectFromNode(const aiScene& scene, const aiNode& node, const vector<shared_ptr<Model>>& meshes, const vector<string>& materials) 
{
	...
	for (unsigned int i = 0; i < node.mNumMeshes; ++i) {
		...
		obj.GetComponent<Mesh>().setFilePath(materials[materialIndex]);
		obj.AddComponent<Material>();
		obj.GetComponent<Material>().m_Texture = std::make_unique<Texture>(materials[materialIndex]);
	...
	for (unsigned int i = 0; i < node.mNumChildren; ++i) {
		obj.emplaceChild(gameObjectFromNode(scene, *node.mChildren[i], meshes, materials));
	}
