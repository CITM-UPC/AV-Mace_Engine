...
{
	ModelLoader loader;
	const std::string fbxFilePath = "Assets/street2.FBX";
	...
	try {
		...
		LOG(LogType::LOG_WARNING, "Error al guardar el archivo FBX en el formato personalizado: %s", e.what());
		return;
	}

	root()->emplaceChild(loader.loadFromFile(fbxFilePath));
...
{
    ModelLoader loader;
    GameObject* rawScene = loader.loadFromFile(path);
    std::shared_ptr<GameObject> scene(rawScene);
    root()->emplaceChild(scene);
...
{
	GameObject camera = GameObject("Camera", "Camera");
	...
	if (selectedGameObject == nullptr) root()->emplaceChild(std::move(camera));
	else selectedGameObject->emplaceChild(std::move(camera));
...
GameObject* Scene::CheckIntersectionRecursive(const Ray& ray, GameObject& object, float& closestT, GameObject* closestObject) {
